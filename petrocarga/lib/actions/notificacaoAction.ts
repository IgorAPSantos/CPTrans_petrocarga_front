...
export async function Notificacao(formData: FormData) {
	const usuarioId = formData.get("usuarioId") as string;
  	...
  	const payload = {
		...
		tipo: formData.get("tipo") as 'RESERVA' | 'VAGA' | 'VEICULO' | 'MOTORISTA' | 'SISTEMA',
		metada: JSON.parse(formData.get("metada") as string)
	};

	const res = await serverApi(`/petrocarga/notificacoes/sendNotification/toUsuario/${usuarioId}`, {
		method: "POST",
...
export async function NotificacaoPorPermissao(formData: FormData) {
  ...
  const payload = {
	...
	const res = await serverApi(`/petrocarga/notificacoes/sendNotification/toUsuario/${permissao}`, {
		method: "POST",
		body: JSON.stringify(payload),
	});
...
export async function getNotificacoesUsuario(usuarioId: string) {
	const res = await serverApi(`/petrocarga/notificacoes/byUsuario/${usuarioId}`, {
		method: "GET",
	});
